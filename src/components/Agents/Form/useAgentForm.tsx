...
export const useAgentForm = (propId?: string) => {
    ...
    const handleSubmit = async () => {
        ...
        // 3. Enrich Data
        try {
            jsonData = setNestedKey(jsonData, ['metadata', 'labels', 'openegiz.agents/name'], agent.name);
            const twinIds = selectedTwins.map((v) => v.value).filter((v): v is string => !!v);
            jsonData = setNestedKey(jsonData, ['metadata', 'labels', 'openegiz.agents/twins'], twinIds);
