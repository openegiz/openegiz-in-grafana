<div align="center">
  <img src="https://github.com/openegiz/openegiz/assets/48439828/74f974ba-3804-46de-9149-2c4fe7702e93" width="90" height="90" />
...
The **OpenEgiz Grafana App Plugin** is a specialized front-end application designed to interface with the **[OpenEgiz Platform](https://github.com/openegiz/openegiz)**.
...
This plugin acts primarily as a frontend client that communicates with the OpenEgiz ecosystem via API calls. It relies on the following architecture:

1.  **Frontend:** This Grafana App Plugin.
2.  **Middleware:** **[Extended API for Eclipse Ditto](https://github.com/openegiz/ditto-extended-api)**. This component acts as an abstraction layer over Eclipse Ditto, adding necessary business logic and validation.
3.  **Core:** **[Eclipse Ditto](https://www.eclipse.org/ditto/)**. The core digital twin framework.

For a comprehensive understanding of the OpenEgiz architecture and how this plugin integrates with the wider ecosystem, please refer to the [OpenEgiz Documentation](https://openegiz.github.io/openegiz/docs/overview/architecture).

> [!NOTE]
> Although the functionality is specific to OpenEgiz, the project structure serves as a valuable template for developers building React-based Grafana App Plugins that interact with external REST APIs.

---

## 🚀 Installation

There are two ways to install this plugin: as part of the full OpenEgiz platform via Kubernetes, or manually into a local Grafana instance.

### Method 1: Full Platform or Helm/Kubernetes
For a complete deployment of the OpenEgiz ecosystem, please refer to the official **[OpenEgiz Documentation](https://openegiz.github.io/openegiz/docs/installation/manual/essential#grafana-v113)**. The Helm charts provided there handle the mounting of this plugin automatically. Users performing a manual installation on Kubernetes or Helm should also consult this guide for specific configuration details.
...
4.  **Configure Grafana:** Since this is a custom plugin, you must allow Grafana to load it. Open your `grafana.ini` (or `custom.ini`) and modify the following setting under `[plugins]`:
    ```ini
    [plugins]
    allow_loading_unsigned_plugins = openegiz-openegiz-app
    ```
...
Before using the plugin, you must configure the connection to your OpenEgiz/Ditto infrastructure.
...
| | **Extended API URL** | Endpoint for the [Extended API](https://github.com/openegiz/ditto-extended-api) middleware component. | **Yes** |
...
> [!WARNING]
> If you are running OpenEgiz locally on Minikube, you must expose the services to make them accessible. Identify the `openegiz-ditto-nginx` and `openegiz-ditto-extended-api` services using `kubectl get services`. Then, run `minikube service <service-name> --url` in separate terminals to generate the accessible localhost URLs needed for the plugin configuration.

---

## 📖 Usage

For detailed instructions on how to use the interface to create and manage digital twins, please refer to the **[OpenEgiz User Documentation](https://openegiz.github.io/openegiz/docs/quickstart)**.
...
1.  **Clone the Repository:**
    ```bash
    git clone https://github.com/openegiz/openegiz-in-grafana.git
    cd grafana-app-openegiz
    ```
